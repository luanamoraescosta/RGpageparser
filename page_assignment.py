import re, json, shutil, xml.etree.ElementTree as ET
...
ENTRY_MARGIN_PX = 70
USE_ENTRY_ALIGNMENT_FILTER = False
ENTRY_RIGHT_TOLERANCE      = 85

MAX_FORWARD_JUMP = 80
MAX_INTERNAL_GAP = 50

INFER_SAFE_SAME_PAGE_MISSING = True
CROSS_PAGE_INFER_MAX_GAP     = 0

# Siglas de arquivo usadas nos Regesta Pontificum
ARCHIVE_SIGLA = {
    "L","S","V","T","A","R","E","Q",
    "IE","II","III","IV","VI","VII","VIII","IX","X",
    "Arm","Reg","Sup","Plut","Chigi","Misc","IE","Reg",
}
...
def load_valid_entry_numbers_from_xml(xml_path: Path) -> set[int]:
    ...
    for lemma in root.iter("lemma"):
        ...
        if m: entries.add(int(m.group(1)))
    return entries
...
def entry_exists_or_fallback(n: int) -> bool:
    if VALID_ENTRY_NUMBERS: return n in VALID_ENTRY_NUMBERS
...
def split_columns(img: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    # Força gutter entre 35% e 65% da largura
...
def ocr_to_data(img_bgr: np.ndarray, config: str = "--psm 6 --oem 3") -> dict:
    ...
    return pytesseract.image_to_data(pil, config=config,
...
def group_into_lines(data: dict, y_tolerance: int = 15) -> dict[int, list[dict]]:
    ...
    for i in range(len(data["text"])):
        ...
        if not word or conf < 20: continue
        y = data["top"][i]
...
def ocr_tokens(data: dict, min_conf: int = 20) -> list[dict]:
    ...
    for i in range(len(data["text"])):
        ...
        conf = safe_conf(data["conf"][i])
        if not raw or conf < min_conf: continue
...
def same_visual_line(a: dict, b: dict) -> bool:
    if vertical_overlap_ratio(a, b) >= 0.35: return True
    # v4: tolerância aumentada de 0.65 → 0.75 para fonts bold
...
def has_token_left_on_same_visual_line(candidate: dict, tokens: list[dict]) -> bool:
    for tok in tokens:
        if tok["index"] == candidate["index"]: continue
        if tok["x"] >= candidate["x"] - 3: continue
        if same_visual_line(candidate, tok): return True
    return False


# ── Page number extraction ───────────────────────────────────────────────────
def _try_extract_page_number(strip: np.ndarray, full_width: int,
    ...
    for _, words in lines.items():
        ...
        if not re.fullmatch(r"\d{1,4}", text): continue
        num = int(text)
        if not (1 <= num <= 2000): continue
...
def parse_entry_number_token(token: str) -> Optional[int]:
    """
    v4: corrige erros OCR em qualquer posição:
    - l/I/| no início (quando seguido de dígito): "l441" → 1441
    - l/I no final (quando precedido de dígito): "44l" → 441
    - O/o → 0 em qualquer posição
    """
...
def parse_entry_number_candidate_token(token: str) -> tuple[Optional[int], Optional[str]]:
    ...
    num_s, rest = m.groups()
    num = int(num_s)
    if not (1 <= num <= entry_upper_bound()): return None, None
    rest = rest.strip(".,;:!?\"'()")
    if not rest or len(rest) <= 1: return None, None
    if re.fullmatch(r"[rvRV]", rest): return None, None
    return num, rest


# ── Archive reference detection (FIX: falsos positivos) ─────────────────────
def has_archive_reference_pattern(next_tokens: list[str]) -> bool:
    """
    Retorna True se os próximos tokens formam uma referência de arquivo,
    não um nome de pessoa. Ex: ['L', '547', '182ss.'] ou ['S', '520', '108v'].
    Usado para filtrar falsos positivos como "59" antes de "L 547 182ᵛss."
    """
    if not next_tokens: return False
    first = next_tokens[0].strip(".,;:")
    # Sigla de arquivo seguida de número
    if first in ARCHIVE_SIGLA and len(next_tokens) >= 2:
        second = next_tokens[1].strip(".,;:")
        if re.search(r"\d", second): return True
    # Padrão de folio/página: número com sufixo r/v/ss
    for tok in next_tokens[:3]:
        if re.search(r"\d+[rvs]{1,3}\.?s?\.?$", tok): return True
    return False
...
def score_name_likeness(tokens: list[str]) -> int:
    ...
    def is_reference_like(tok: str) -> bool:
        raw = clean_token(tok); stripped = raw.strip(".")
        if not stripped: return True
        if re.search(r"\d", stripped): return True
        if stripped in reference_sigla: return True
        if re.fullmatch(r"[IVXLCDM]+", stripped): return True
        if stripped.isupper() and len(stripped) <= 4: return True
        return False

    def is_strong_name(tok: str) -> bool:
        raw = clean_token(tok)
        if not raw or re.search(r"\d", raw) or is_reference_like(raw): return False
        normalized = normalize_candidate(raw)
        if not normalized: return False
        low = normalized.lower().strip(".")
        if low in stopwords: return False
        if abbreviation_pattern.match(normalized): return True
        stripped_norm = normalized.strip(".")
        if len(stripped_norm) < 3: return False
        if normalized.isupper() and len(normalized) <= 4: return False
        if re.match(rf"^[{upper}][{lower}]{{2,}}", normalized): return True
        if re.match(rf"^[{upper}][{lower}]{{1,}}", normalized): return True
        if "(" in raw and len(stripped_norm) >= 4 and re.match(rf"^[{lower}]{{4,}}", normalized): return True
        return False
    ...
    for tok in candidate_tokens:
        if is_strong_name(tok): score += 5
    if len(tokens) >= 2 and is_strong_name(tokens[1]): score += 2
    for tok in tokens[:2]:
        if "(" in tok and is_strong_name(tok): score += 1
    for tok in tokens[:3]:
        if is_reference_like(tok): score -= 3
    return max(score, 0)


# ── Entry extraction ──────────────────────────────────────────────────────────
def _extract_entries_from_column(
    col: np.ndarray,
    conf_min: int = OCR_CONF_MIN,
    name_score_min: int = 5,
) -> list[int]:
    found: list[int] = []
    ...
    for config in TESS_CONFIGS:
        ...
        for tok in tokens:
            if tok["conf"] < conf_min: continue
            entry_num, attached_name = parse_entry_number_candidate_token(tok["text"])
            if entry_num is None: continue
            if tok["x"] > margin_limit + 10: continue
            if has_token_left_on_same_visual_line(tok, tokens): continue

            right_tokens = tokens_right_on_same_visual_line(tok, tokens, max_tokens=8)
            next_tokens: list[str] = []
            if attached_name: next_tokens.append(attached_name)
            next_tokens.extend(t["text"] for t in right_tokens if t["text"].strip())

            # FIX: rejeita referências de arquivo (ex: "59 L 547 182ss.")
        ...
        if pass_found: break  # sucesso no primeiro PSM que achar entradas
...
def extract_entry_numbers(img: np.ndarray) -> list[int]:
    ...
    for col in (left_col, right_col):
        ...
        if not entries:
            # Fallback: confiança menor, score mínimo mais baixo
...
def normalize_page_numbers(infos: list[PageInfo]) -> None:
    ...
    for i, info in enumerate(infos):
        if info.page_number is not None and 1 <= info.page_number <= 2000:
            anchor_idx = i; anchor_page = info.page_number; break
    if anchor_idx is None:
        print("  [page normalization] No reliable anchor page found."); return
...
def correct_by_expected_suffix(n: int, previous_max: int,
    ...
    suffixes = [(raw, 0)]
    if len(raw) >= 3: suffixes.append((raw[-3:], 1))
    if len(raw) >= 2: suffixes.append((raw[-2:], 2))

    candidates = []
    for cand in range(lo, hi + 1):
        if cand <= previous_max or cand in same_page_others: continue
        if VALID_ENTRY_NUMBERS and cand not in VALID_ENTRY_NUMBERS: continue
        cand_s = str(cand)
...
def clean_entry_numbers(infos: list[PageInfo]) -> None:
    ...
    for info in infos:
        ...
        cleaned: list[int] = []
        for n in temp:
            if not cleaned: cleaned.append(n); continue
            gap = n - cleaned[-1]
...
def report_duplicate_entries(infos: list[PageInfo]) -> None:
    ...
    for info in infos:
        for entry in info.entry_numbers:
            if entry in seen: duplicates.append((entry, seen[entry], info.filename))
            else: seen[entry] = info.filename
    print("\n=== DUPLICATE ENTRIES ===")
    if not duplicates: print("✓ No duplicate entries after cleaning.")
    else:
        for entry, ff, sf in duplicates[:50]:
            print(f"  entry {entry}: {ff} AND {sf}")
        if len(duplicates) > 50: print(f"  ... and {len(duplicates) - 50} more")
...
def report_entry_gaps_by_scan(infos: list[PageInfo]) -> None:
    ...
    if not warnings: print("✓ No suspicious entry gaps/jumps.")
    else:
...
def report_missing_entries(mapping: dict[int, int], full_domain: bool = True) -> None:
    print("\n=== MISSING ENTRIES ===")
    if full_domain and VALID_ENTRY_NUMBERS:
        expected_entries = sorted(VALID_ENTRY_NUMBERS)
    else:
        entries = sorted(mapping)
        if not entries: print("No entries in mapping."); return
        expected_entries = list(range(entries[0], entries[-1] + 1))
    missing = [n for n in expected_entries if n not in mapping]
    if not missing: print("✓ No missing entries.")
    else:
        print(f"Missing entries: {len(missing)}")
        print(missing[:200])
        if len(missing) > 200: print(f"... and {len(missing) - 200} more")

def save_missing_entries_context(mapping: dict[int, int], full_domain: bool = True) -> None:
    entries = sorted(mapping)
    if not entries: print("No entries in mapping."); return
    expected_entries = sorted(VALID_ENTRY_NUMBERS) if (full_domain and VALID_ENTRY_NUMBERS) \
                       else list(range(entries[0], entries[-1]+1))
    missing = [n for n in expected_entries if n not in mapping]
...
def infer_missing_entries(mapping: dict[int, int], max_gap_entries: int = 100) -> dict[int, int]:
    ...
    Does NOT infer across page boundaries:
      440 -> p.70
      442 -> p.71
      441 remains ambiguous.
    """
    ...
    expected_entries = (
        sorted(VALID_ENTRY_NUMBERS)
        if VALID_ENTRY_NUMBERS
        else list(range(min(mapping), max(mapping) + 1))
    )
    ...
    for left_entry, right_entry in zip(known_entries, known_entries[1:]):
        ...
        # Only infer same-page gaps.
        if left_page != right_page:
            continue

        missing_between = [
            n for n in range(left_entry + 1, right_entry)
    ...
    with open(out, "w", encoding="utf-8") as f:
        ...
        for entry, page, left_entry, right_entry, reason in added:
            f.write(
                f"{entry}\t{page}\t{left_entry}\t{right_entry}\t{reason}\n"
            )
    ...
    for entry, page, left_entry, right_entry, reason in added[:40]:
        print(
            f"  {entry} -> {page} "
            f"[{reason}] between {left_entry} and {right_entry}"
        )
...
def report_ambiguous_missing_between_pages(mapping: dict[int, int],
                                            full_domain: bool = True) -> None:
    """
    Report missing entries that are between known entries on different pages.

    These are NOT auto-filled, because they may be at the end of the previous
    page or at the beginning of the next page.
    ...
    Then:
      441 is ambiguous between p.70 and p.71.
    """
    ...
    for left_entry, right_entry in zip(known_entries, known_entries[1:]):
        ...
        # Same-page gaps are inferred elsewhere.
        if left_page == right_page:
            continue
    ...
    out = DEBUG_DIR / "ambiguous_missing_between_pages.tsv"

    with open(out, "w", encoding="utf-8") as f:
        f.write(
            "missing_entry\tleft_entry\tleft_page\t"
            "right_entry\tright_page\tcomment\n"
        )
    ...
    for row in rows[:40]:
        n, left_entry, left_page, right_entry, right_page, _ = row

        print(
            f"  {n}: between {left_entry}(p.{left_page}) "
            f"and {right_entry}(p.{right_page})"
        )

    if len(rows) > 40:
        print(f"  ... and {len(rows) - 40} more")
# ── Mapping builder ───────────────────────────────────────────────────────────
def build_page_mapping(force_rebuild: bool = False,
                        max_images: Optional[int] = None) -> dict[int, int]:
    ...
    for info in infos:
        if info.page_number is None: continue
        for entry in info.entry_numbers:
...
# ── Manual overrides ──────────────────────────────────────────────────────────
def apply_manual_overrides(mapping: dict[int, int]) -> dict[int, int]:
    if not MANUAL_OVERRIDES.exists():
        print(f"\nNo manual overrides found: {MANUAL_OVERRIDES}"); return mapping
    with open(MANUAL_OVERRIDES, "r", encoding="utf-8") as f:
        raw = json.load(f)
    overrides = {int(k): int(v) for k, v in raw.items()}
    print("\n=== APPLYING MANUAL OVERRIDES ===")
    for entry, page in sorted(overrides.items()):
        if not entry_exists_or_fallback(entry):
            print(f"  entry {entry}: ignored"); continue
        old = mapping.get(entry)
        mapping[entry] = page
        if old is None: print(f"  entry {entry}: added page {page}")
        elif old != page: print(f"  entry {entry}: {old} -> {page}")
        else: print(f"  entry {entry}: already page {page}")
    return dict(sorted(mapping.items()))


# ── XML enrichment ────────────────────────────────────────────────────────────
def enrich_xml(mapping: dict[int, int]) -> None:
    """
    For each <lemma id="...NNNN">, derive the entry number from the
    last 4 digits of the lemma id, look up the book page, and insert/update:

        <page source="ocr">N</page>

    directly inside <sublemma>, not inside <reg>.
    ...
    for lemma in root.iter("lemma"):
        ...
        if not entry_exists_or_fallback(entry_num):
            continue

        page = mapping.get(entry_num)

        if page is None:
            missing.append((lemma_id, entry_num))
            continue

        # In your XML, sublemma is inside reg
        sublemma = lemma.find("./reg/sublemma")

        # Fallback: find sublemma anywhere under lemma
        if sublemma is None:
            sublemma = lemma.find(".//sublemma")

        if sublemma is None:
            no_sublemma.append((lemma_id, entry_num))
            continue

        # Find page directly under sublemma
        ...
        page_el.set("source", "ocr")

        # Optional cleanup:
        # remove old <page> inside <reg>, if previous versions inserted it there.
        reg = lemma.find("reg")
        if reg is not None:
            old_reg_page = reg.find("page")
            if old_reg_page is not None:
                reg.remove(old_reg_page)
    ...
    if XML_OUTPUT.exists():
        ...
        backup = XML_OUTPUT.with_name(
            XML_OUTPUT.stem + f"_backup_{timestamp}" + XML_OUTPUT.suffix
        )
    ...
    tmp_output = XML_OUTPUT.with_suffix(".tmp.xml")
    ...
    tmp_output.replace(XML_OUTPUT)

    print(f"\n{'=' * 60}")
    print(f"Enriched sublemmas : {enriched}")
    print(f"No page in mapping : {len(missing)}")
    print(f"No sublemma found  : {len(no_sublemma)}")

    if missing:
        print("\nMissing page mapping examples:")
        for lemma_id, entry_num in missing[:20]:
            print(f"  id={lemma_id}  entry={entry_num}")

        if len(missing) > 20:
            print(f"  … and {len(missing) - 20} more")

    if no_sublemma:
        print("\nNo sublemma examples:")
        for lemma_id, entry_num in no_sublemma[:20]:
            print(f"  id={lemma_id}  entry={entry_num}")

        if len(no_sublemma) > 20:
            print(f"  … and {len(no_sublemma) - 20} more")
...
def validate_mapping(mapping: dict[int, int], sample_size: int = 20) -> None:
    import random
    print("\n=== MAPPING VALIDATION ===")
    if not mapping: print("No mapping entries."); return
    keys = sorted(mapping)
    sample = sorted(random.sample(keys, min(sample_size, len(keys))))
    print(f"{'Entry':>6}  →  {'Page':>5}\n" + "-"*22)
    for k in sample: print(f"{k:>6}  →  {mapping[k]:>5}")
...
if __name__ == "__main__":
    if TEST_MODE:
        mapping = build_page_mapping(force_rebuild=False, max_images=TEST_MAX_IMAGES)
        mapping = infer_missing_entries(mapping)
        report_ambiguous_missing_between_pages(mapping, full_domain=False)
        validate_mapping(mapping)
        report_missing_entries(mapping, full_domain=False)
        save_missing_entries_context(mapping, full_domain=False)
        save_mapping_report(mapping)
        print("\nTEST MODE: XML was NOT enriched.")
    else:
        mapping = build_page_mapping(force_rebuild=False)
        mapping = infer_missing_entries(mapping)
        report_ambiguous_missing_between_pages(mapping, full_domain=True)
        mapping = apply_manual_overrides(mapping)
        validate_mapping(mapping)
        report_missing_entries(mapping, full_domain=True)
        save_missing_entries_context(mapping, full_domain=True)
        save_mapping_report(mapping)
        enrich_xml(mapping)
